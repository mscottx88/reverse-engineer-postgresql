...
WHERE (
  to_regnamespace('source') = CON.connamespace
  AND CLS.relname = COALESCE(NULL, CLS.relname)
)
AND NOT EXISTS (
  SELECT
...
FROM (
  ...
  WHERE (
    to_regnamespace('source') = CLS.relnamespace
...
WHERE table_name = COALESCE(NULL, table_name)
...
WHERE to_regnamespace('source') = TYP.typnamespace
GROUP BY typname
ORDER BY statement
LIMIT 100
OFFSET 0;
...
FROM pg_proc PRC
WHERE to_regnamespace('source') = PRC.pronamespace
ORDER BY statement
LIMIT 100
OFFSET 0;
...
FROM pg_indexes IDX
WHERE (
  IDX.schemaname = 'source'
  AND IDX.tablename = COALESCE(NULL, IDX.tablename)
  AND NOT EXISTS (
    SELECT
    FROM pg_constraint CON
    WHERE (
...
WHERE (
  PN.nspname = 'source'
  AND PRN.relname = COALESCE(NULL, PRN.relname)
  AND CON.contype = 'c'
)
ORDER BY statement
LIMIT 100
...
WHERE NSP.nspname = 'source'
AND REF.relname = COALESCE(NULL, REF.relname)
AND CLS.relkind = 'S'
ORDER BY statement
LIMIT 100
OFFSET 0;
